...
- 未追跡ファイル: `.claude/` のみ（コミットするかは判断が要る）
- **README.md が create-next-app のテンプレートのまま。** このアプリが何なのかがどこにも書かれていない
  （`pnb-manual` のように README + CLAUDE.md を書くと、次に触るときに楽になる）
...
1. **README を書く**（何のアプリか・本番URL・セットアップ・デプロイ手順）。余裕があれば CLAUDE.md も
2. `.claude/` を追跡するか決める
3. Firebase 移行後の動作確認が済んでいるか（旧 Prisma 時代の機能で消えたものがないか）の棚卸し
